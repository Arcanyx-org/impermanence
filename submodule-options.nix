  ...
  commonOpts = {
    options = {
      ...
      stripHomePrefix = mkOption {
        type = bool;
        default = config.stripHomePrefix;
        defaultText = "environment.persistence.‹name›.stripHomePrefix";
        description = ''
          Whether to strip the home directory prefix from the persistent
          storage path. When enabled, directories like "Desktop" are stored
          directly under persistentStoragePath/Desktop instead of
          persistentStoragePath/home/user/Desktop.
        '';
      };
    };
  };
  ...
  dir = submodule ([
    ...
    ({ config, ... }:
      ...
      in
      {
        ...
        sourcePath = if config.stripHomePrefix then config.directory else concatPaths [ home config.directory ];
      })
